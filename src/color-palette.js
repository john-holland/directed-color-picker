...
const workBoots = new WorkBoots({ socksFile: href + 'k-means-clustering-socks.js' });
	...
	progressCallback = (progress) => { console.log("... progress! " + progress); }
		...
		runUpload(files[0]).then(image => {
			...
			workBoots.ready().then(() => {
				...
				paletteSize = (isNaN(paletteSize) || +paletteSize === 0) ? 10 : paletteSize
